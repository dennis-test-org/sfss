...
class TestAuthFunctions(unittest.TestCase):
    ...
    def test_get_github_username(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            Config.API_URL, headers={"Authorization": "token dummy_token"}
        )
    ...
    def test_authenticate_success(
        ...
        mock_instance.auth_state = (
            "aaaaaaaaaaaaaaaa"  # Ensure this matches the generated state in the test
        )
    ...
    def test_save_token(self, mock_logger, mock_file, mock_makedirs):
        ...
        mock_makedirs.assert_called_once_with(
            os.path.join(Config.TOKENS_DIR), exist_ok=True
        )
        mock_file.assert_called_once_with(
            os.path.join(Config.TOKENS_DIR, "testuser.env"), "w"
        )
    ...
    def test_load_token(self, mock_file, mock_exists):
        ...
        mock_exists.assert_called_once_with(
            os.path.join(Config.TOKENS_DIR, "testuser.env")
        )
        mock_file.assert_called_once_with(
            os.path.join(Config.TOKENS_DIR, "testuser.env"), "r"
        )
    ...
    def test_is_authenticated_success(
        self, mock_logger, mock_load_token, mock_load_active_user, mock_datetime
    ):
        mock_datetime.now.return_value = datetime(
            2023, 10, 12, 12, 37, 56, tzinfo=timezone.utc
        )
        mock_datetime.fromisoformat.return_value = datetime(
            2023, 10, 12, 12, 34, 56, tzinfo=timezone.utc
        )
    ...
    def test_is_authenticated_expired(
        ...
        mock_datetime.now.return_value = datetime(
            2023, 10, 12, 12, 10, 1, tzinfo=timezone.utc
        )
        mock_datetime.fromisoformat.return_value = datetime(
            2023, 10, 12, 12, 00, 00, tzinfo=timezone.utc
        )
        result = is_authenticated()
